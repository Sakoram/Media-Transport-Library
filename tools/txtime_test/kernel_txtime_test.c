  ...
  while (1) {
    ...
    for (cmsg = CMSG_FIRSTHDR(&msg); cmsg; cmsg = CMSG_NXTHDR(&msg, cmsg)) {
      if (cmsg->cmsg_level == SOL_IP && cmsg->cmsg_type == IP_RECVERR) {
        ...
        if (count <= 10)
          printf(
              "  ERRQUEUE[%d]: ee_errno=%d ee_origin=%d ee_type=%d "
              "ee_code=%d ee_info=%u\n",
              count, serr->ee_errno, serr->ee_origin, serr->ee_type, serr->ee_code,
              serr->ee_info);
      }
    }
  }
  if (count > 10) printf("  ... (%d more errors)\n", count - 10);
  if (count == 0) printf("  (none)\n");
}

/* Send one packet with SO_TXTIME via UDP sendmsg */
static int send_txtime_pkt(int fd, const struct sockaddr_in* dst, uint32_t seq,
                           uint64_t tai_base, uint64_t launch_ns) {
  ...
  while ((opt = getopt_long(argc, argv, "i:I:c:o:", opts, NULL)) != -1) {
    switch (opt) {
      ...
      case 'I':
        ...
        break;
      case 'c':
        g_count = (uint32_t)atoi(optarg);
        break;
      case 'o':
        g_offset_ms = (uint32_t)atoi(optarg);
        break;
    }
  }
}
...
int main(int argc, char** argv) {
  ...
  for (size_t j = 0; j < SIG_LEN; j++)
    printf("    gap[%zu] = %u ns (%u us)\n", j, SIG_PATTERN[j], SIG_PATTERN[j] / 1000);
  printf("========================================\n\n");
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_TXTIME, &so_txtime, sizeof(so_txtime)) < 0) {
    ...
    fprintf(stderr,
            "Hint: Is ETF qdisc configured on %s?\n"
  ...
  printf("\nCLOCK_TAI    : %" PRIu64 " ns  (%u.%09u)\n", tai_base,
         (unsigned)(tai_base / 1000000000ULL), (unsigned)(tai_base % 1000000000ULL));
  ...
  for (uint32_t i = 0; i < g_count; i++) {
    ...
    if (i == 0 || i == g_count - 1 || (i % 10 == 0)) {
      ...
      printf("  pkt[%4u] launch_delta=%+" PRId64 " us  gap=%" PRIu64 " us  %s\n", i,
             delta / 1000, gap_ns[i] / 1000, ret > 0 ? "OK" : "FAIL");
    }
  }
  ...
  for (int s = 1; s <= 3; s++) {
    sleep(1);
    uint64_t tai_now = clock_tai_ns();
    printf("  +%ds: TAI=%u.%09u\n", s, (unsigned)(tai_now / 1000000000ULL),
