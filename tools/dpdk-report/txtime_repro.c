...
static int test_without_txtime(uint16_t port, struct rte_mempool* pool) {
  ...
  printf("tx_burst: %u / 3\n", sent);
  for (int i = sent; i < 3; i++) rte_pktmbuf_free(pkts[i]);
...
static int test_with_txtime(uint16_t port, struct rte_mempool* pool) {
  ...
  for (int i = 0; i < NUM_PKTS; i++) {
    ...
    *RTE_MBUF_DYNFIELD(pkts[i], ts_offset, uint64_t*) = launch + (uint64_t)i * 1024;
  }

  uint16_t sent = rte_eth_tx_burst(port, 0, pkts, NUM_PKTS);
  printf("tx_burst: %u / %d\n", sent, NUM_PKTS);
  for (int i = sent; i < NUM_PKTS; i++) rte_pktmbuf_free(pkts[i]);
...
int main(int argc, char* argv[]) {
  ...
  if (!pool) rte_exit(EXIT_FAILURE, "pool create failed\n");
  ...
  if (result == 0)
    ...
    printf(
        "FAIL: opackets=0 — launch-time packets never "
        "transmitted\n");
