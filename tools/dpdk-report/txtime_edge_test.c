...
static int port_init(void) {
  ...
  ret = rte_eth_rx_queue_setup(g_port, 0, RX_DESC, rte_eth_dev_socket_id(g_port), NULL,
                               g_pool);
  if (ret < 0) return ret;
  ...
  ret =
      rte_eth_tx_queue_setup(g_port, 0, TX_DESC, rte_eth_dev_socket_id(g_port), &txconf);
  if (ret < 0) return ret;
 ...
 */
static struct rte_mbuf* build_pkt(uint16_t test_id, uint16_t seq, int64_t offset_ns,
                                  uint64_t phc_now) {
  ...
  struct rte_ipv4_hdr* ip =
      (struct rte_ipv4_hdr*)rte_pktmbuf_append(m, sizeof(struct rte_ipv4_hdr));
  ...
  struct rte_udp_hdr* udp =
      (struct rte_udp_hdr*)rte_pktmbuf_append(m, sizeof(struct rte_udp_hdr));
  udp->src_port = rte_cpu_to_be_16(UDP_PORT);
  udp->dst_port = rte_cpu_to_be_16(UDP_PORT);

  /* Payload */
  struct test_payload* p =
      (struct test_payload*)rte_pktmbuf_append(m, sizeof(struct test_payload));
  ...
  for (int t = 0; t < n_tests; t++) {
    ...
    if (t == 22) {
      ...
      for (int s = 1; s <= WAIT_SEC; s++) {
        ...
        rte_eth_stats_get(g_port, &s1);
        printf("    +%ds opkts=%" PRIu64 " oerr=%" PRIu64 "\n", s,
               s1.opackets - s0.opackets, s1.oerrors - s0.oerrors);
      }
      rte_eth_stats_get(g_port, &s1);
