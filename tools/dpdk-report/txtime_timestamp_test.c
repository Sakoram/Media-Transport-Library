...
static int port_init(uint16_t port, struct rte_mempool* pool) {
  ...
  /* Let PHC stabilize */
  for (int i = 0; i < 100; i++) {
    if (read_phc(port) > 1000000000ULL) break;
    usleep(50000);
  }
  return 0;
}
...
static uint64_t send_pkts(uint64_t ts_val, const char* label) {
  ...
  for (int i = 0; i < BURST; i++) {
    ...
    *RTE_MBUF_DYNFIELD(pkts[i], ts_offset, uint64_t*) = ts_val + (uint64_t)i * 128;
  }

  sent = rte_eth_tx_burst(g_port, 0, pkts, BURST);
  for (int i = sent; i < BURST; i++) rte_pktmbuf_free(pkts[i]);
  ...
  for (int s = 1; s <= WAIT_SEC; s++) {
    ...
    printf("    +%ds opkts=%" PRIu64 " obytes=%" PRIu64 " oerrs=%" PRIu64 "\n", s,
           s1.opackets - s0.opackets, s1.obytes - s0.obytes, s1.oerrors - s0.oerrors);
  }
  ...
  printf("    %s: ts_val=%" PRIu64 " sub_s=%" PRIu64 "ns PHC_sub=%" PRIu64
         "ns  burst=%u delta=%" PRIu64 "\n",
...
static int check(const char* tag, uint64_t got, int expect_nonzero) {
  ...
  printf("  >> %s: %" PRIu64 " packets -- %s (expected %s)\n\n", tag, got,
         ok ? "OK" : "**UNEXPECTED**", expect_nonzero ? ">= 5" : "0");
  return ok ? 1 : 0;
}
...
int main(int argc, char* argv[]) {
  ...
  ts_offset = rte_mbuf_dynfield_register(&(struct rte_mbuf_dynfield){
  ...
  g_port = 0;
  g_pool =
      rte_pktmbuf_pool_create("POOL", NUM_MBUFS, MBUF_CACHE, 0, RTE_MBUF_DEFAULT_BUF_SIZE,
  ...
  phc = read_phc(g_port);
  total++;
  passed += check("Step A", send_pkts(phc + 1000000, "PHC+1ms"), 1);

  printf("  Step B: PHC + 5 ms  (queue still healthy)\n");
  phc = read_phc(g_port);
  total++;
  passed += check("Step B", send_pkts(phc + 5000000, "PHC+5ms"), 1);
  ...
  total++;
  passed += check("Step C", send_pkts(0, "dynfield=0"), 0);

  printf("  Step D: PHC + 1 ms  (valid ts, but queue should be stuck)\n");
  phc = read_phc(g_port);
  total++;
  passed += check("Step D", send_pkts(phc + 1000000, "PHC+1ms STUCK?"), 0);
  ...
  phc = read_phc(g_port);
  total++;
  passed += check("Step F", send_pkts(phc + 1000000, "PHC+1ms RECOVERED"), 1);
